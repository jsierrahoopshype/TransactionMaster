...
<head>
  ...
  <style>
    ...
    * {
    ...
    .rm-banner-icon { font-size: 1.1rem; }
    ...
    .rm-send-btn:hover { background: #f0f0f2; transform: translateY(-1px); }
    .rm-send-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
    .rm-send-btn.sent { background: #34c759; color: white; }
    @media (max-width: 600px) {
      .rm-banner { flex-wrap: wrap; }
      .rm-send-btn { width: 100%; text-align: center; margin-top: 0.25rem; }
    }
      font-weight: 500;
      color: var(--accent);
    ...
    .realistic-meter-header {
      display: flex;
      align-items: center;
      justify-content: space-between;
      margin-bottom: 0.5rem;
    }
    .realistic-meter-label {
      font-size: 0.75rem;
      font-weight: 600;
      color: var(--text-secondary);
      text-transform: uppercase;
    ...
    .realistic-meter-fill {
      height: 100%;
      border-radius: 5px;
      transition: width 0.3s ease, background 0.3s ease;
    }
    .meter-very-unbalanced { background: #ef4444; }
    .meter-unbalanced { background: #eab308; }
    .meter-fair { background: #86efac; }
    .meter-balanced { background: #22c55e; }
    ...
    .ntc-badge, .restriction-badge, .na-badge, .pp-badge, .min-badge, .tw-badge, .opt-badge, .exp-badge {
      font-size: 0.5625rem;
      padding: 0.125rem 0.3rem;
      border-radius: 3px;
      font-weight: 600;
...
<body>
  <div class="container">
    <header>
      <h1><a href="https://hoopsmatic.com" style="text-decoration: none; color: inherit;">🏀 NBA Trade Machine</a></h1>
      <div class="header-actions">
        <div class="header-row">
          <div class="trade-machine unified-trade-machine">
            ...
            <!-- Generate button -->
            <button class="btn btn-primary" id="generateTradeBtn">🎲 Generate</button>
          </div>
        </div>
        <div class="header-row">
          <button class="btn btn-loop" id="loopTradeBtn" title="Auto-generate new trade every 8 seconds">▶ Trade Loop</button>
          <button class="btn btn-share-loop" id="shareLoopBtn" title="Copy link to share this trade loop" style="display: none;">🔗 Share Loop</button>
          <button class="btn btn-success" id="generateImageBtn">📸 Generate Image</button>
      ...
      <div class="image-modal-content">
        <div class="image-modal-header">
          ...
          <div class="format-selector">
            <button class="format-btn active" data-format="twitter" title="Twitter/X (16:9)">𝕏</button>
            <button class="format-btn" data-format="instagram" title="Instagram (1:1)">◻</button>
            <button class="format-btn" data-format="stories" title="Stories (9:16)">▯</button>
          </div>
        ...
        <div class="image-modal-footer">
          <button class="btn btn-primary" id="downloadImageBtn">⬇️ Download Image</button>
          <button class="btn btn-secondary" id="copyImageBtn">📋 Copy to Clipboard</button>
        </div>
      </div>
    </div>
    ...
    <div id="mainContent">
      <!-- Roster Manager integration banner -->
      <div class="rm-banner" id="rmBanner" style="display: none;">
        <span class="rm-banner-icon">🔄</span>
        <span class="rm-banner-text">Build your trade here, then send it back to Roster Manager</span>
        <button class="rm-send-btn" id="rmSendBtn" disabled onclick="sendToRosterManager()">Build a valid trade first</button>
      </div>
      <div class="trade-status" id="tradeStatus" style="display: none;">
        ...
        <a href="#tradeRumorsContainer" class="check-rumors-link" id="checkRumorsLink" style="display: none;">📰 Check Rumors</a>
      </div>
      
      <!-- Realistic Meter -->
      <div class="realistic-meter" id="realisticMeter" style="display: none;">
        <div class="realistic-meter-header">
          <span class="realistic-meter-label">⚖️ Trade Balance</span>
          <span class="realistic-meter-value" id="realisticMeterValue">Balanced</span>
        </div>
        <div class="realistic-meter-bar">
          <div class="realistic-meter-fill" id="realisticMeterFill" style="width: 100%;"></div>
        </div>
      </div>
      ...
      <div class="glossary-section">
        ...
        <div class="glossary-grid">
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Two-Way Contract</strong> — Counts as $0 in trades</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>10-Day Contract</strong> — Cannot be traded</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>No-Trade Clause</strong> — Player can veto trades</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Not Aggregated</strong> — Can't be packaged with other salaries</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Poison Pill</strong> — Incoming value is average of current-year salary and the annual salary in each year of extension</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Trade Kicker</strong> — Player gets bonus % on trade</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Min Salary Exception</strong> — No salary matching needed</span>
          </div>
          <div class="glossary-item">
          ...
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Trade Restriction</strong> — Date restriction before trade</span>
          </div>
          <div class="glossary-item">
            <span class="glossary-badge"><span class="opt-badge" style="padding: 0.125rem 0.4rem; border-radius: 4px; font-size: 0.65rem; font-weight: 600;">TO</span></span>
            <span class="glossary-description"><strong>Team Option</strong> — Team option for 26-27, only tradeable if exercised</span>
          </div>
          <div class="glossary-item">
            <span class="glossary-badge"><span class="opt-badge" style="padding: 0.125rem 0.4rem; border-radius: 4px; font-size: 0.65rem; font-weight: 600;">PO</span></span>
            <span class="glossary-description"><strong>Player Option</strong> — Player option for 26-27, only tradeable if exercised</span>
          </div>
          <div class="glossary-item">
            <span class="glossary-badge"><span class="exp-badge" style="padding: 0.125rem 0.4rem; border-radius: 4px; font-size: 0.65rem; font-weight: 600;">EXP</span></span>
            <span class="glossary-description"><strong>Expiring</strong> — Contract expires this season, not tradeable</span>
          </div>
          <div class="glossary-item">
            <span class="glossary-badge"><span class="injury-badge" style="padding: 0 0.15rem; border-radius: 2px; font-size: 0.65rem; font-weight: 700; color: #ff0000; background: white; border: 1.5px solid #ff0000;">✚</span></span>
            <span class="glossary-description"><strong>Inactive</strong> — Player is currently out</span>
          </div>
          <div class="glossary-item">
            ...
            <span class="glossary-description"><strong>Conditional Pick</strong> — Pick has conditions</span>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    console.log('%c🏀 NBA Trade Machine v17.4 - Feb 8 2026', 'color: #22c55e; font-weight: bold; font-size: 14px');
    console.log('Features: Pick Swaps, TPE multi-player, NA decomposition, Offseason mode (expiring/options)');
    ...
    // Roster Manager integration mode
    const IS_RM_MODE = INITIAL_URL_PARAMS.has('rm');
    const RM_TEAM = INITIAL_URL_PARAMS.get('rm') || ''; // team abbreviation from Roster Manager
    ...
    const tradeValueFetchPromise = fetch(TV_CSV_URL)
      ...
      .then(csvText => {
        ...
        // Re-render trade status to update realistic meter with trade values
        if (typeof renderTradeStatus === 'function') {
          renderTradeStatus();
        } else {
        }
      })
      .catch(err => {
        // Silently handle trade value fetch errors
      });
    ...
    function computeTradeValues(csvText) {
      ...
      function calcPotentialBonus(age, draftPick) {
        ...
        // Draft position factor: exponential decay (pick 1 = 1.0, pick 60 ≈ 0.05)
        // Bigger gap between pick 1-2 than between pick 59-60
        const draftFactor = Math.exp(-DRAFT_DECAY * (draftPick - 1));
        
        // Age factor: exponential decay (18.82 years = 1.0, 25 years ≈ 0.04)
    ...
    const FIRST_ROUND_PICK_TV = 10000000;
    const SECOND_ROUND_PICK_TV = 3000000;
    
    function getPickTradeValue(pick) {
      const baseValue = pick.round === '1st' ? FIRST_ROUND_PICK_TV : SECOND_ROUND_PICK_TV;
      // Swaps are worth less because you might not get the better pick
      // Assume ~50% value since it's uncertain which pick you'll end up with
      return pick.isSwap ? baseValue * 0.5 : baseValue;
    }
    ...
    // Calculate trade balance percentage for the realistic meter (0-100%)
    // Constants for trade balance adjustments
    const MINIMUM_SALARY = 1272870; // For roster release penalty
    
    // Best player premium scales from 0% to 25% based on trade value
    // - Below $5M TV: 0% premium (rotation/minimum players)
    // - $5M to $80M TV: linear scale from 0% to 25%
    // - Above $80M TV: 25% premium cap (superstars like Jokic)
    const BEST_PLAYER_MIN_TV = 5000000;   // Below this = 0% premium
    const BEST_PLAYER_MAX_TV = 80000000;  // Above this = 25% premium
    const BEST_PLAYER_MAX_PREMIUM = 0.25; // Maximum 25% bonus
    
    function getBestPlayerPremium(tradeValue) {
      if (tradeValue <= BEST_PLAYER_MIN_TV) return 0;
      if (tradeValue >= BEST_PLAYER_MAX_TV) return BEST_PLAYER_MAX_PREMIUM;
      // Linear interpolation between min and max
      const ratio = (tradeValue - BEST_PLAYER_MIN_TV) / (BEST_PLAYER_MAX_TV - BEST_PLAYER_MIN_TV);
      return ratio * BEST_PLAYER_MAX_PREMIUM;
    }
    
    function calculateTradeBalance(trade) {
      if (!trade || trade.length < 2) return { percentage: 0, details: null };
      if (tradeValueMapLower.size === 0) return { percentage: 100, details: null };
      
      // Find the best player in the entire trade (highest TV among all players being traded)
      let bestPlayer = null;
      let bestPlayerTV = 0;
      let bestPlayerFromTeamIdx = -1;
      ...
      trade.forEach((t, idx) => {
        t.playersOut.forEach(p => {
          ...
          if (tv > bestPlayerTV) {
            bestPlayerTV = tv;
            bestPlayer = p.player;
            bestPlayerFromTeamIdx = idx;
          }
        });
      });
      
      // Calculate roster release penalty for each team
      const rosterPenalties = trade.map((t, idx) => {
        // Match team name (tradeTeams uses short names like "Denver", players use full names like "Denver Nuggets")
        const fullTeamName = teams.find(team => team.includes(t.teamName) || t.teamName.includes(team.split(' ')[0]));
        if (!fullTeamName) {
          return 0;
        }
        
        // Count current standard roster (excluding two-way and 10-day)
        const currentStandardCount = players.filter(p => 
          p.team === fullTeamName &&
          p.contractStatus?.toUpperCase() !== 'TWO-WAY' &&
          !p.contractStatus?.toUpperCase().includes('10-DAY')
        ).length;
        
        // Count outgoing standard players
        const outgoingStandard = t.playersOut.filter(p =>
          p.contractStatus?.toUpperCase() !== 'TWO-WAY' &&
          !p.contractStatus?.toUpperCase().includes('10-DAY')
        ).length;
        
        // Count incoming standard players
        const incomingStandard = t.playersIn.filter(p =>
          p.contractStatus?.toUpperCase() !== 'TWO-WAY' &&
          !p.contractStatus?.toUpperCase().includes('10-DAY')
        ).length;
        ...
        // If roster exceeds 15, team must release players
        if (postTradeCount > 15) {
          const mustRelease = postTradeCount - 15;
          const penalty = mustRelease * MINIMUM_SALARY * 2;
          return penalty;
        }
        return 0;
      });
      
      const teamData = trade.map((t, idx) => {
        // Calculate outgoing player values with filler discount
        // Step 1: Find the main player (highest TV) for this team
        let mainPlayerTV = 0;
        let mainPlayerName = '';
        t.playersOut.forEach(p => {
          const tv = getTradeValue(p.player) || p.salary;
          if (tv > mainPlayerTV) {
            mainPlayerTV = tv;
            mainPlayerName = p.player;
          }
        });
        
        // Step 2: Calculate total with filler discount for secondary players
        const playersOutTV = t.playersOut.reduce((sum, p) => {
          const tv = getTradeValue(p.player) || p.salary;
          
          // If this is the main player, use full value
          if (p.player === mainPlayerName || t.playersOut.length === 1) {
            return sum + tv;
          }
          
          // For secondary players, apply filler discount
          // Discount scales from 0% (if equal to main) to 50% (if very small)
          // Formula: multiplier = 0.5 + 0.5 * (playerTV / mainTV)
          const ratio = mainPlayerTV > 0 ? tv / mainPlayerTV : 1;
          const fillerMultiplier = 0.3 + 0.7 * Math.min(ratio, 1);
          const adjustedTV = tv * fillerMultiplier;
          return sum + adjustedTV;
        }, 0);
        const picksOutTV = (t.picksOut || []).reduce((sum, pick) => {
          return sum + getPickTradeValue(pick);
        }, 0);
        // Add roster release penalty to outgoing value
        let totalOut = playersOutTV + picksOutTV + rosterPenalties[idx];
        
        // Add best player premium to the team SENDING the best player
        // This increases their "cost" because they're giving up a star
        if (bestPlayerFromTeamIdx === idx) {
          totalOut += bestPlayerTV * getBestPlayerPremium(bestPlayerTV);
        }
        
        // Calculate incoming player values with filler discount
        // Step 1: Find the main incoming player (highest TV)
        let mainIncomingTV = 0;
        let mainIncomingName = '';
        t.playersIn.forEach(p => {
          const tv = getTradeValue(p.player) || p.salary;
          if (tv > mainIncomingTV) {
            mainIncomingTV = tv;
            mainIncomingName = p.player;
          }
        });
        
        // Step 2: Calculate total with filler discount for secondary incoming players
        let playersInTV = t.playersIn.reduce((sum, p) => {
          const tv = getTradeValue(p.player) || p.salary;
          
          // If this is the main player, use full value
          if (p.player === mainIncomingName || t.playersIn.length === 1) {
            return sum + tv;
          }
          
          // For secondary players, apply filler discount
          const ratio = mainIncomingTV > 0 ? tv / mainIncomingTV : 1;
          const fillerMultiplier = 0.3 + 0.7 * Math.min(ratio, 1);
          const adjustedTV = tv * fillerMultiplier;
          return sum + adjustedTV;
        }, 0);
        
        // Add best player premium if this team is receiving the best player
        if (bestPlayerFromTeamIdx !== -1 && bestPlayerFromTeamIdx !== idx) {
          // Check if this team is receiving the best player
          const receivingBestPlayer = t.playersIn.some(p => p.player === bestPlayer);
          if (receivingBestPlayer) {
            playersInTV += bestPlayerTV * getBestPlayerPremium(bestPlayerTV);
          }
        }
        
        const picksInTV = trade.reduce((sum, otherTeam, otherIdx) => {
          if (otherIdx === idx) return sum;
          return sum + (otherTeam.picksOut || [])
            .filter(pick => pick.destination === idx)
            .reduce((pickSum, pick) => pickSum + getPickTradeValue(pick), 0);
        }, 0);
        const totalIn = playersInTV + picksInTV;
        
        return { teamName: t.teamName, out: totalOut, in: totalIn };
      });
      
      // For all trades (2-team or more), compare each team's out vs in value
      // This properly factors in best player premium and roster penalties
      let worstRatio = 1.0;
      for (const team of teamData) {
        if (team.out === 0 && team.in === 0) continue;
        if (team.out === 0 || team.in === 0) { worstRatio = Math.min(worstRatio, 0.3); continue; }
        // Only penalize teams that are LOSING value (out > in)
        // Teams that are winning (in > out) get ratio = 1.0
        const ratio = team.out > team.in ? (team.in / team.out) : 1.0;
        worstRatio = Math.min(worstRatio, ratio);
      }
      const finalPercentage = Math.round(worstRatio * 100);
      return { percentage: finalPercentage, details: teamData };
    }
    
    // Update the realistic meter UI
    function updateRealisticMeter(isLegalTrade = false) {
      const meter = document.getElementById('realisticMeter');
      const fill = document.getElementById('realisticMeterFill');
      const value = document.getElementById('realisticMeterValue');
      
      
      if (!meter || !fill || !value) return;
      
      const hasAnyAssets = tradeTeams.some(t => 
        (t.playersOut && t.playersOut.length > 0) ||
        (t.picksOut && t.picksOut.length > 0)
      );
      
      if (!hasAnyAssets || tradeValueMapLower.size === 0 || !isLegalTrade) {
        meter.style.display = 'none';
        return;
      }
      
      meter.style.display = 'block';
      const balance = calculateTradeBalance(tradeTeams);
      const pct = balance.percentage;
      
      
      fill.style.width = pct + '%';
      fill.classList.remove('meter-very-unbalanced', 'meter-unbalanced', 'meter-fair', 'meter-balanced');
      
      if (pct < 40) {
        fill.classList.add('meter-very-unbalanced');
        value.textContent = 'Very Unbalanced';
        value.style.color = '#ef4444';
      } else if (pct < 60) {
        fill.classList.add('meter-unbalanced');
        value.textContent = 'Unbalanced';
        value.style.color = '#eab308';
      } else if (pct < 85) {
        fill.classList.add('meter-fair');
        value.textContent = 'Fair';
        value.style.color = '#86efac';
      } else {
        fill.classList.add('meter-balanced');
        value.textContent = 'Balanced';
        value.style.color = '#22c55e';
      }
    }
    
    // Check Stepien Rule for a test trade (not using global tradeTeams)
    function checkStepienForTestTrade(testTrade, teamIndex) {
      const team = testTrade[teamIndex];
      const teamName = team.teamName;
      const picksOut = team.picksOut || [];
      
      // Only count full trades, not swaps
      const firstRoundPicksOut = picksOut.filter(p => p.round === '1st' && !p.isSwap);
      if (firstRoundPicksOut.length === 0) return true;
      
      const teamFirstRoundPicks = draftPicks
        .filter(p => p.team === teamName && p.round === '1st')
        .map(p => parseInt(p.year));
      
      const yearsOut = firstRoundPicksOut.map(p => parseInt(p.year));
      
      const incomingYears = [];
      testTrade.forEach((otherTeam, otherIdx) => {
        if (otherIdx !== teamIndex && otherTeam.picksOut) {
          otherTeam.picksOut.forEach(pick => {
            if (pick.round === '1st' && pick.destination === teamIndex) {
              incomingYears.push(parseInt(pick.year));
            }
          });
        }
      });
      
      const remainingYears = teamFirstRoundPicks
        .filter(y => !yearsOut.includes(y))
      ...
      for (let i = 0; i < years.length - 1; i++) {
        ...
        if (!remainingYears.includes(year1) && !remainingYears.includes(year2)) {
          return false;
        }
      }
      return true;
    ...
    function getAvailablePicksForRandomTrade(teamName, existingPicksOut = []) {
      ...
      // Only consider non-conditional (sure) picks that are not untradeable
      const surePicks = teamPicks.filter(p => 
        p.surePick === 'YES' && 
        !(p.notes && p.notes.toLowerCase().includes('untradeable'))
      );
    ...
    function addPicksToTrade(testTrade, teamIndex, destination, isSuperstar) {
      ...
      // Add first round picks one at a time, rechecking Stepien after each
      for (let i = 0; i < maxFirstRound; i++) {
        ...
        if (availablePicks.firstRound.length > 0) {
          const pick = availablePicks.firstRound[Math.floor(Math.random() * availablePicks.firstRound.length)];
          testTrade[teamIndex].picksOut.push({
            team: pick.team,
            year: pick.year,
            round: pick.round,
            notes: pick.notes || '',
    ...
    // Check if a trade with NA players can be decomposed into valid sub-trades
    // Each NA player must form an independent valid "mini-trade" with incoming players
    // Remaining aggregable players must form a valid trade with remaining incoming players
    function checkTradeDecomposition(tradeTeam, tradeTeams, teamIndex) {
      const naPlayers = tradeTeam.playersOut.filter(p => p.notAggregated);
      const regularPlayers = tradeTeam.playersOut.filter(p => !p.notAggregated);
      
      if (naPlayers.length === 0) return true; // No NA players, no decomposition needed
      
      // Get team info for salary matching calculations
      const teamInfo = getTeamInfo(tradeTeam.teamName);
      if (!teamInfo) return false;
      const status = teamInfo.status?.toUpperCase() || 'BELOW TAX';
      
      // Helper to calculate max incoming for a given outgoing salary
      function calcMaxIncoming(outgoing, teamStatus) {
        if (teamStatus === 'CAP SPACE') return Infinity; // Simplified - cap space teams very flexible
        if (teamStatus === '1ST APRON' || teamStatus === '2ND APRON') return outgoing;
        if (teamStatus === 'TAX TEAM') return outgoing * 1.25 + 100000;
        // Below Tax tiers
        if (outgoing <= 7500000) return outgoing * 2 + 250000;
        if (outgoing <= 29000000) return outgoing + 7500000;
        return outgoing * 1.25 + 250000;
      }
      
      // For simplicity, focus on 2-team trades (most common case)
      // Get all incoming players to this team
      let incomingPlayers = [];
      tradeTeams.forEach((t, idx) => {
        if (idx !== teamIndex && t.playersOut) {
          t.playersOut.forEach(p => {
            if (p.destination === teamIndex) {
              incomingPlayers.push({ ...p, fromTeamIdx: idx });
            }
          });
        }
      });
      
      if (incomingPlayers.length === 0) return false; // No incoming players to match
      
      // Try to find a valid decomposition
      // For each NA player, try to match them with a subset of incoming players
      // Use a greedy approach: for each NA player, find the smallest subset that works
      
      // Sort incoming players by salary (smallest first) for greedy matching
      const sortedIncoming = [...incomingPlayers].sort((a, b) => getIncomingSalary(a) - getIncomingSalary(b));
      
      let usedIncoming = new Set();
      
      for (const naPlayer of naPlayers) {
        const naSalary = getOutgoingSalary(naPlayer);
        const naMaxIncoming = calcMaxIncoming(naSalary, status);
        
        // Find incoming players that can match this NA player
        // The NA player can receive salary up to naMaxIncoming
        // And each incoming player being sent to this NA player must be able to match
        let found = false;
        
        // Try single player matches first (most common)
        for (let i = 0; i < sortedIncoming.length; i++) {
          if (usedIncoming.has(i)) continue;
          
          const inPlayer = sortedIncoming[i];
          const inSalary = getIncomingSalary(inPlayer);
          
          // Check: can this NA player receive this incoming player?
          if (inSalary > naMaxIncoming) continue;
          
          // Check: can the incoming player's team send this player to receive the NA player?
          const inTeam = tradeTeams[inPlayer.fromTeamIdx];
          const inTeamInfo = getTeamInfo(inTeam?.teamName);
          if (!inTeamInfo) continue;
          
          const inStatus = inTeamInfo.status?.toUpperCase() || 'BELOW TAX';
          const inMaxIncoming = calcMaxIncoming(inSalary, inStatus);
          
          // The incoming player's team receives the NA player
          if (naSalary <= inMaxIncoming) {
            // Match found!
            usedIncoming.add(i);
            found = true;
            break;
        ...
        if (!found) {
          // Try combinations of 2 incoming players
          for (let i = 0; i < sortedIncoming.length; i++) {
            if (usedIncoming.has(i)) continue;
            for (let j = i + 1; j < sortedIncoming.length; j++) {
              if (usedIncoming.has(j)) continue;
              
              const inSalary1 = getIncomingSalary(sortedIncoming[i]);
              const inSalary2 = getIncomingSalary(sortedIncoming[j]);
              const combinedInSalary = inSalary1 + inSalary2;
              
              if (combinedInSalary <= naMaxIncoming) {
                // Check reverse: can these players' team(s) receive the NA player?
                // Simplified: assume same team for now
                const inTeam = tradeTeams[sortedIncoming[i].fromTeamIdx];
                const inTeamInfo = getTeamInfo(inTeam?.teamName);
                if (inTeamInfo) {
                  const inStatus = inTeamInfo.status?.toUpperCase() || 'BELOW TAX';
                  const inMaxIncoming = calcMaxIncoming(combinedInSalary, inStatus);
                  if (naSalary <= inMaxIncoming) {
                    usedIncoming.add(i);
                    usedIncoming.add(j);
                    found = true;
        ...
        if (!found) return false; // Couldn't find valid match for this NA player
      }
      
      // Now check if remaining regular players can match remaining incoming players
      const remainingIncoming = sortedIncoming.filter((_, i) => !usedIncoming.has(i));
      const regularOutgoing = regularPlayers.reduce((sum, p) => sum + getOutgoingSalary(p), 0);
      const remainingIncomingSalary = remainingIncoming.reduce((sum, p) => sum + getIncomingSalary(p), 0);
      
      if (regularPlayers.length === 0 && remainingIncoming.length === 0) {
        return true; // Perfect match
      }
      
      if (regularPlayers.length === 0 && remainingIncoming.length > 0) {
        return false; // Leftover incoming with no one to match
      }
      
      if (remainingIncoming.length === 0 && regularPlayers.length > 0) {
        return false; // Leftover outgoing with no one to receive
      }
      
      // Check if regular players can match remaining incoming
      const regularMaxIncoming = calcMaxIncoming(regularOutgoing, status);
      if (remainingIncomingSalary > regularMaxIncoming) return false;
      
      // Check reverse: can remaining incoming players' team receive regular players?
      // Get the team sending the remaining incoming (assume same team for simplicity)
      if (remainingIncoming.length > 0) {
        const inTeam = tradeTeams[remainingIncoming[0].fromTeamIdx];
        const inTeamInfo = getTeamInfo(inTeam?.teamName);
        if (inTeamInfo) {
          const inStatus = inTeamInfo.status?.toUpperCase() || 'BELOW TAX';
          const inMaxIncoming = calcMaxIncoming(remainingIncomingSalary, inStatus);
          if (regularOutgoing > inMaxIncoming) return false;
        }
      }
      
      return true; // Decomposition found!
    }
    ...
    function validateTeamTrade(tradeTeam) {
      ...
      // EXCEPTION 1: NA players can be traded if the receiving team absorbs them into a TPE
      // EXCEPTION 2: NA players can be in multi-player trades if the trade can be decomposed into valid sub-trades
      const naPlayers = tradeTeam.playersOut.filter(p => p.notAggregated);
      const naPlayersNotAbsorbed = naPlayers.filter(p => {
        if (p.destination === undefined) return true;
        const destTeam = tradeTeams[p.destination];
        if (!destTeam) return true;
        
        if (destTeam.tpeUsed && destTeam.tpeUsed.value >= getOutgoingSalary(p)) {
          return false;
        }
        
        const destTPEs = getTeamTPEs(destTeam.teamName);
        const canBeAbsorbed = destTPEs.some(tpe => tpe.value >= getOutgoingSalary(p));
        if (canBeAbsorbed) {
          return false;
        }
      ...
      // NEW: Check if trade can be decomposed into valid sub-trades
      let canDecompose = false;
      if (naPlayersNotAbsorbed.length > 0 && tradeTeam.playersOut.length > 1) {
        // Find this team's index in tradeTeams
        const teamIndex = tradeTeams.findIndex(t => t.teamName === tradeTeam.teamName);
        if (teamIndex >= 0) {
          canDecompose = checkTradeDecomposition(tradeTeam, tradeTeams, teamIndex);
        }
      }
      
      if (naPlayersNotAbsorbed.length > 0 && tradeTeam.playersOut.length > 1 && !canDecompose) {
        naPlayersNotAbsorbed.forEach(p => {
      ...
      tradeTeam.playersOut.forEach(p => {
        ...
        if (p.isExpiring) {
          restrictions.push(`${p.player} has an expiring contract (not tradeable)`);
        } else if (p.tradeAvailable && p.tradeAvailable.toLowerCase() === 'not available') {
          restrictions.push(`${p.player} cannot be traded`);
        } else if (p.tradeAvailable) {
          restrictions.push(`${p.player} available ${p.tradeAvailable}`);
        }
        // Check for options
        if (p.hasTeamOption) {
          restrictions.push(`${p.player} has a Team Option - only tradeable if option exercised`);
        }
        if (p.hasPlayerOption) {
          restrictions.push(`${p.player} has a Player Option - only tradeable if option exercised`);
        }
        if (p.noTradeClause) {
      ...
      // If there are NA violations (and players can't be absorbed into TPE or decomposed), trade is invalid
      if (naPlayersNotAbsorbed.length > 0 && tradeTeam.playersOut.length > 1 && !canDecompose) {
        return {
          valid: false,
          message: `${naPlayersNotAbsorbed.map(p => p.player).join(', ')} cannot be aggregated with other players`,
    ...
    function validateStepienRule(teamName, teamIndex) {
      ...
      // Only check first-round picks being FULLY traded out (not swaps)
      // Swaps don't violate Stepien because team still has A pick in that year
      const firstRoundPicksOut = picksOut.filter(p => p.round === '1st' && !p.isSwap);
      
      // Get incoming first-round picks from other teams (including swaps they're receiving)
    ...
    function validateTrade() {
      ...
      tradeTeams.forEach(t => {
        if (t.teamName && t.tpeUsed && t.playersIn.length > 0 && t.playersOut.length === 0) {
          ...
          // Total incoming salary must fit within TPE value
          const totalIncoming = t.playersIn.reduce((sum, p) => sum + getIncomingSalary(p), 0);
          if (totalIncoming > t.tpeUsed.value) {
            allValid = false;
            invalidMessages.push(`${t.teamName}: Incoming salary ($${(totalIncoming/1000000).toFixed(1)}M) exceeds TPE value ($${(t.tpeUsed.value/1000000).toFixed(1)}M)`);
          }
        ...
        // If team IS sending players but ALSO using TPE, check that the salary absorbed by TPE doesn't exceed TPE value
        if (t.teamName && t.tpeUsed && t.playersIn.length > 0 && t.playersOut.length > 0) {
          const outgoingSalary = t.playersOut.reduce((sum, p) => sum + getOutgoingSalary(p), 0);
          const incomingSalary = t.playersIn.reduce((sum, p) => sum + getIncomingSalary(p), 0);
          
          // Calculate max incoming without TPE
          const teamInfo = getTeamInfo(t.teamName);
          if (teamInfo) {
            const status = teamInfo.status?.toUpperCase() || 'BELOW TAX';
            let maxIncomingWithoutTPE;
            if (status === 'CAP SPACE') {
              maxIncomingWithoutTPE = (teamInfo.capSpace || 0) + outgoingSalary;
            } else if (status === 'TAX TEAM') {
              maxIncomingWithoutTPE = outgoingSalary * 1.25 + 100000;
            } else if (status === '1ST APRON' || status === '2ND APRON') {
              maxIncomingWithoutTPE = outgoingSalary;
            } else {
              if (outgoingSalary <= 7500000) {
                maxIncomingWithoutTPE = outgoingSalary * 2 + 250000;
              } else if (outgoingSalary <= 29000000) {
                maxIncomingWithoutTPE = outgoingSalary + 7500000;
              } else {
                maxIncomingWithoutTPE = outgoingSalary * 1.25 + 250000;
              }
            }
            
            // The amount that must be absorbed by TPE = incoming - maxIncomingWithoutTPE
            const tpeNeeded = incomingSalary - maxIncomingWithoutTPE;
            if (tpeNeeded > 0 && tpeNeeded > t.tpeUsed.value) {
              allValid = false;
              invalidMessages.push(`${t.teamName}: Salary absorbed by TPE ($${(tpeNeeded/1000000).toFixed(1)}M) exceeds TPE value ($${(t.tpeUsed.value/1000000).toFixed(1)}M)`);
            }
          }
        }
    ...
    // Auto-add TPE if it would make a trade legal
    function autoAddTPEIfNeeded() {
      tradeTeams.forEach((team, teamIndex) => {
        if (!team.teamName || team.playersIn.length === 0) return;
        if (team.tpeUsed) return; // Already has TPE selected
        
        const teamInfo = getTeamInfo(team.teamName);
        if (!teamInfo) return;
        
        // Check team status - Apron teams can't use TPEs
        const status = teamInfo.status?.toUpperCase() || 'BELOW TAX';
        if (status === '1ST APRON' || status === '2ND APRON') return;
        
        // Calculate current salary situation
        ...
        const deficit = incomingSalary - maxIncoming;
        if (deficit <= 0) return; // Trade already works
        
        // Find all TPEs that can cover the deficit and pick the smallest one
        const teamTPEs = getTeamTPEs(team.teamName);
        const availableTPEs = teamTPEs.filter(tpe => {
          // Check if TPE is already used by another team in this trade
          const isUsedElsewhere = tradeTeams.some((t, i) => 
            i !== teamIndex && t.tpeUsed && 
            t.tpeUsed.value === tpe.value && 
            t.tpeUsed.team === tpe.team &&
            t.tpeUsed.expirationDate === tpe.expirationDate
          );
          if (isUsedElsewhere) return false;
          
          // TPE must cover the deficit
          return tpe.value >= deficit;
        });
        
        // Sort by value (smallest first) and pick the smallest sufficient TPE
        availableTPEs.sort((a, b) => a.value - b.value);
        const usableTPE = availableTPEs[0];
        
        if (usableTPE) {
          team.tpeUsed = usableTPE;
          console.log(`🔄 Auto-added TPE: ${team.teamName} using $${(usableTPE.value/1000000).toFixed(1)}M TPE to cover $${(deficit/1000000).toFixed(1)}M deficit`);
        }
      });
    }
    ...
    function redistributePlayers(addAutoCash = false) {
      ...
      // Auto-add TPE if it would make the trade legal
      autoAddTPEIfNeeded();
            ...
            <div class="trade-content" data-trade-content="${index}">
              ...
              `}
                      ...
                      <div class="player-chip-info">
                        <div class="player-chip-name">
                          ...
                          <span class="player-info-wrapper">
                            <span class="player-info-icon">ℹ</span>
                            <div class="player-info-tooltip">
                              <div class="player-info-name">${p.player}</div>
                              <div class="player-info-team">${p.team}${p.position ? ` • ${p.position}` : ''}</div>
                              ...
                              <div class="player-info-row">
                                ...
                                <span class="player-info-value"><span style="color: ${getRatingColor(p.globalRating)}">${p.globalRating ? p.globalRating.toFixed(1) : 'N/A'} RAT</span>${p.gamesPlayed ? ` • <span style="color: ${getGamesColor(p.gamesPlayed)}">${p.gamesPlayed} G</span>` : ''}</span>
                              </div>
                              <div class="player-info-row">
                          ...
                          ${tradePct >= 3 ? `<span class="trending-badge" title="Trending: Appears in ${tradePct}% of trades">🔥</span>` : ''}
                          ${p.isOut ? `<span class="injury-badge" title="${p.injuryReason || 'Inactive'}">✚</span>` : ''}
                          ...
                          ${p.hasTeamOption ? '<span class="opt-badge" title="Team Option for 26-27 - Only tradeable if option exercised">TO</span>' : ''}
                          ${p.hasPlayerOption ? '<span class="opt-badge" title="Player Option for 26-27 - Only tradeable if option exercised">PO</span>' : ''}
                          ${p.isExpiring ? '<span class="exp-badge" title="Expiring Contract - Not tradeable after deadline">EXP</span>' : ''}
                          ${p.tradeAvailable && !p.isExpiring ? '<span class="restriction-badge" title="Trade Restriction">!</span>' : ''}
                        </div>
                        <div class="player-chip-salary">${formatSalary(outgoingValue)}${isTwoWay && p.salary > 0 ? ` <span style="color: #6366f1; font-size: 0.7em;">(actual ${formatSalary(p.salary)})</span>` : ''}${p.isMinSalaryException ? ` <span style="color: #10b981; font-size: 0.7em;">→ $0 incoming</span>` : ''}${p.poisonPill > 0 ? ` <span style="color: #ec4899; font-size: 0.7em;">→ ${formatSalary(p.poisonPill)}</span>` : ''}${p.isMinimum && !p.poisonPill && !p.isMinSalaryException ? ` <span style="color: #22c55e; font-size: 0.7em;">→ ${formatSalary(LEAGUE_MINIMUM)}</span>` : ''}</div>
                      </div>
                ...
                ${(tradeTeam.picksOut || []).map((pick, pickIdx) => {
                  ...
                  const isSwap = pick.isSwap || false;
                  return `
                    <div class="player-chip" style="background: ${isSwap ? 'var(--purple-dim)' : 'var(--orange-dim)'}; border-color: ${isSwap ? 'var(--purple)' : 'var(--orange)'};">
                      <div class="player-chip-row">
                        <div class="player-chip-info">
                          <div class="player-chip-name" style="color: ${isSwap ? 'var(--purple)' : 'var(--orange)'};">
                            📋 ${pick.year} ${pick.round} Round Pick ${isSwap ? '<span class="swap-badge">SWAP</span>' : ''}
                      ...
                      ` : ''}
                      <div class="player-destination">
                        <span class="player-destination-label">🔄</span>
                        <select class="player-destination-select swap-select" data-team="${index}" data-pick="${pickIdx}">
                          <option value="trade" ${!isSwap ? 'selected' : ''}>Full Trade</option>
                          <option value="swap" ${isSwap ? 'selected' : ''}>Swap Rights</option>
                        </select>
                      </div>
                    </div>
                  `;
              ...
              <div class="players-in">
                ...
                ${incomingPicks.map(pick => {
                  ...
                  const pickSource = (pick.notes && !pick.notes.toLowerCase().includes('own pick')) 
                    ...
                    : `${pick.fromTeam} pick`;
                  return `
                    <div class="player-chip incoming" style="background: ${isSwap ? 'var(--purple-dim)' : 'var(--orange-dim)'}; border-color: ${isSwap ? 'var(--purple)' : 'var(--orange)'};">
                      <div class="player-chip-row">
                        <div class="player-chip-info">
                          <div class="player-chip-name" style="color: ${isSwap ? 'var(--purple)' : 'var(--orange)'};">
                            📋 ${pick.year} ${pick.round} Round Pick ${isSwap ? '<span class="swap-badge">SWAP</span>' : ''}
                      ...
                      return `
                        <div class="player-roster-item ${isDisabled ? 'disabled' : ''}" data-team="${index}" data-player="${p.player}" ${isDisabled ? 'data-disabled="true"' : ''}>
                          ...
                          <span class="player-roster-name">
                            ...
                            ${p.hasPlayerOption ? '<span class="opt-badge" title="Player Option for 26-27 - Only tradeable if option exercised">PO</span>' : ''}
                            ${p.isExpiring ? '<span class="exp-badge" title="Expiring Contract - Not tradeable after deadline">EXP</span>' : ''}
                            ${p.tradeAvailable && !is10Day && !p.isExpiring ? '<span class="restriction-badge" title="Trade Restriction">!</span>' : ''}
                            ${(() => {
                              const pct = getPlayerTradePercentage(p.playerId);
                              if (pct >= 3) {
                                return `<span class="trending-badge" title="Trending: Appears in ${pct}% of trades">🔥</span>`;
                              }
                              return '';
                            })()}
                          </span>
                    ...
                    <div class="picks-section">
                      ...
                      <div class="picks-list">
                        ${teamPicks.map((pick, pickIndex) => {
                          ...
                          const isConditional = pick.surePick !== 'YES';
                          const isUntradeable = pick.notes && pick.notes.toLowerCase().includes('untradeable');
                          return `
                            <div class="pick-item ${isSelected ? 'selected' : ''} ${isUsedElsewhere || isUntradeable ? 'disabled' : ''}" 
                                 data-team="${index}" 
                                 ...
                                 ${isUsedElsewhere || isUntradeable ? 'data-disabled="true"' : ''}
                                 title="${isUntradeable ? 'This pick cannot be traded' : (pick.notes || `${pick.year} ${pick.round} Round Pick`)}">
                              <div>
    ...
    function renderTradeStatus() {
      ...
      // Update the realistic meter (only for legal trades)
      const isLegalTrade = validation.valid || validation.type?.startsWith('pending');
      updateRealisticMeter(isLegalTrade);
      
      // Update Roster Manager send button if in RM mode
      if (IS_RM_MODE) {
        const rmBtn = document.getElementById('rmSendBtn');
        if (rmBtn) {
          if (isLegalTrade) {
            rmBtn.disabled = false;
            rmBtn.textContent = '✅ Send Trade to Roster Manager';
            rmBtn.classList.remove('sent');
          } else {
            rmBtn.disabled = true;
            rmBtn.textContent = 'Build a valid trade first';
            rmBtn.classList.remove('sent');
          }
        }
      }
    ...
    // ========== ROSTER MANAGER INTEGRATION ==========
    function sendToRosterManager() {
      if (!IS_RM_MODE) return;
      
      // Find the RM team in the trade
      const rmTeamFull = RM_TEAM ? (abbrevToTeam[RM_TEAM.toUpperCase()] || '') : '';
      
      // Collect all players going OUT from the RM team and IN to the RM team
      const playersOut = [];
      const playersIn = [];
      
      for (const t of tradeTeams) {
        if (!t.teamName) continue;
        const isRmTeam = t.teamName === rmTeamFull || teamAbbreviations[t.teamName] === RM_TEAM.toUpperCase();
        
        if (isRmTeam) {
          // Players this team is sending away
          for (const p of t.playersOut) {
            playersOut.push({
              name: p.player,
              salary: p.salary,
              pos: p.position || '',
          ...
          // Players this team is receiving
          for (const p of t.playersIn) {
            playersIn.push({
              name: p.player,
              salary: p.salary,
              pos: p.position || '',
              headshot: p.headshot || '',
              playerId: p.playerId || '',
              fromTeam: teamAbbreviations[p.team] || p.team
            });
          }
        }
      }
      ...
      const tradeData = {
        type: 'ROSTER_MANAGER_TRADE',
        team: RM_TEAM.toUpperCase(),
        out: playersOut,
        in: playersIn,
        timestamp: Date.now()
      };
      
      // Send to opener (popup mode) or parent (iframe mode)
      const target = window.opener || window.parent;
      if (target && target !== window) {
        target.postMessage(tradeData, '*');
        
        // Visual feedback
        const btn = document.getElementById('rmSendBtn');
        if (btn) {
          btn.textContent = '✓ Sent! You can close this window';
          btn.classList.add('sent');
          btn.disabled = true;
        }
      } else {
        // Fallback: copy trade data to clipboard as JSON
        navigator.clipboard.writeText(JSON.stringify(tradeData, null, 2)).then(() => {
          alert('Trade data copied to clipboard. Paste it in Roster Manager.');
        }).catch(() => {
          alert('Could not send trade. Please make sure Roster Manager is open.');
        });
      }
    }
    
    // Show RM banner on load if in RM mode
    if (IS_RM_MODE) {
      const rmBanner = document.getElementById('rmBanner');
      if (rmBanner) rmBanner.style.display = 'flex';
    }
    ...
    function updateUrlAndTitle() {
      ...
      // Encode picks (format: fromTeamIdx:pickTeamAbbrev:year:round:destIdx:isSwap,...)
      if (allPicks.length > 0) {
        const pickStrings = allPicks.slice(0, 6).map(pick => {
          ...
          // Add swap flag (s for swap, empty for full trade)
          const swapFlag = pick.isSwap ? 's' : '';
          return `${pick.fromTeamIndex}:${pickTeamAbbrev}:${pick.year}:${roundShort}:${pick.destination !== undefined ? pick.destination : ''}${swapFlag}`;
        });
    ...
    function attachEventListeners() {
      ...
      document.querySelectorAll('.swap-select').forEach(select => {
        select.addEventListener('change', (e) => {
          const teamIndex = parseInt(e.target.dataset.team);
          const pickIndex = parseInt(e.target.dataset.pick);
          const pick = tradeTeams[teamIndex].picksOut[pickIndex];
          pick.isSwap = e.target.value === 'swap';
          render();
        });
      });
        ...
        item.addEventListener('click', () => {
          ...
          if (existingIndex >= 0) {
            ...
            tradeTeams[teamIndex].picksOut.push({
              ...
              isSwap: false // Default to full trade, can toggle to swap
            });
          }
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        if (isThreeTeam) {
          ...
          if (team1Valid.valid && team2Valid.valid && team3Valid.valid &&
            ...
            // Maybe add draft picks (30% chance for each team in 3-team trades)
            for (let side = 0; side < 3; side++) {
              if (Math.random() < 0.30) {
                const possibleDests = [0, 1, 2].filter(t => t !== side);
                const dest = possibleDests[Math.floor(Math.random() * possibleDests.length)];
                addPicksToTrade(testTrade, side, dest, false);
              }
            }
            
            // Final validation: ensure picks don't violate Stepien Rule
            const finalStepienCheck = testTrade.every((t, idx) => {
              if (!t.picksOut || t.picksOut.length === 0) return true;
              return checkStepienForTestTrade(testTrade, idx);
            });
            
            if (!finalStepienCheck) {
              testTrade.forEach(t => t.picksOut = []);
            }
          ...
          for (const player2 of shuffled) {
            ...
            if (team1Valid.valid && team2Valid.valid && team1Apron.valid && team2Apron.valid && isTradeValueBalanced(testTrade)) {
              ...
              // Maybe add draft picks (40% chance for each team)
              for (let side = 0; side < 2; side++) {
                if (Math.random() < 0.40) {
                  const otherSide = side === 0 ? 1 : 0;
                  addPicksToTrade(testTrade, side, otherSide, false);
                }
              }
    ...
    function renderUnifiedPlayerChips() {
      ...
      container.innerHTML = unifiedSelectedPlayers.map((p, idx) => `
        <span class="selected-player-chip">
          ${p.player}
          <span class="selected-player-chip-remove" data-idx="${idx}">×</span>
        </span>
      `).join('');
    ...
    function renderExcludedChips() {
      excludedChipsContainer.innerHTML = excludedPlayers.map(name => `
        <span class="excluded-chip" data-player="${name}">
          ${name}
          <span class="excluded-chip-remove" data-player="${name}">×</span>
        </span>
      `).join('');
    ...
    shareLoopBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        shareLoopBtn.textContent = '✓ Copied!';
        shareLoopBtn.classList.add('copied');
        setTimeout(() => {
          shareLoopBtn.textContent = '🔗 Share Loop';
    ...
    function stopLoop() {
      ...
      loopTradeBtn.textContent = '▶ Trade Loop';
    ...
    copyImageBtn.addEventListener('click', async () => {
      try {
        ...
        setTimeout(() => {
          copyImageBtn.textContent = '📋 Copy to Clipboard';
        }, 2000);
      } catch (err) {
    ...
    async function generateTradeImage() {
      ...
      // Trade status badge
      const validation = validateTrade();
      const statusText = validation.valid ? '✓ VALID TRADE' : '✕ INVALID';
      ...
      displayTeams.forEach((team, teamIdx) => {
        ...
        if (sendsOutCount > 0) {
          ...
          if (team.picksOut) {
            team.picksOut.forEach(pick => {
              const isSwap = pick.isSwap || false;
              // Use purple for swaps, orange for full trades
              ctx.fillStyle = isSwap ? '#f3e8ff' : '#fef3c7';
              ctx.beginPath();
              ctx.roundRect(x + 12, currentY, teamWidth - 24, assetRowHeight - 6, 8);
              ctx.fill();
              
              ctx.strokeStyle = isSwap ? '#d8b4fe' : '#fcd34d';
              ctx.lineWidth = 1;
              ctx.beginPath();
              ctx.roundRect(x + 12, currentY, teamWidth - 24, assetRowHeight - 6, 8);
              ctx.stroke();
              
              ctx.fillStyle = isSwap ? '#6b21a8' : '#92400e';
              ...
              const swapText = isSwap ? ' (SWAP)' : '';
              const pickText = pickOrigin 
                ? `📋 ${pick.year} ${pick.round} (${pickOrigin})${swapText}` 
                : `📋 ${pick.year} ${pick.round} Round Pick${swapText}`;
          ...
          if (team.rightsOut) {
            team.rightsOut.forEach(rights => {
              ...
              ctx.font = '600 12px -apple-system, BlinkMacSystemFont, sans-serif';
              ctx.textAlign = 'left';
              ctx.fillText(`🎓 Rights to ${rights.playerName}`, x + 20, currentY + 23);
              currentY += assetRowHeight;
            });
          }
          ...
          if (team.cashOut) {
            team.cashOut.forEach(() => {
              ...
              ctx.font = '600 12px -apple-system, BlinkMacSystemFont, sans-serif';
              ctx.textAlign = 'left';
              ctx.fillText('💵 Cash Considerations', x + 20, currentY + 23);
              currentY += assetRowHeight;
            });
          }
          ...
          if (usesTPE > 0 && team.tpeUsed) {
            ...
            ctx.font = '600 12px -apple-system, BlinkMacSystemFont, sans-serif';
            ctx.textAlign = 'left';
            ctx.fillText(`📄 TPE: ${formatSalary(tpeAmountUsed)}`, x + 20, currentY + 23);
            currentY += assetRowHeight;
          }
        ...
        if (receivesCount > 0) {
          ...
          incomingPicks.forEach(pick => {
            ...
            ctx.font = '600 12px -apple-system, BlinkMacSystemFont, sans-serif';
            ctx.textAlign = 'left';
            ctx.fillText(`📋 ${pick.year} ${pick.round} Round Pick${isSwap ? ' (SWAP)' : ''}`, x + 20, currentY + 23);
            currentY += assetRowHeight;
          });
    ...
    function removeOutletFromText(text, outlet) {
      ...
      const patterns = [
        // Exact outlet at end
        new RegExp('\\s*[-–—]?\\s*' + outletClean.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s*$', 'i'),
        // With domain extension
        new RegExp('\\s*[-–—]?\\s*' + outletClean.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\.(com|org|net)\\s*$', 'i'),
        // Full outlet name
        new RegExp('\\s*[-–—]?\\s*' + outlet.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s*$', 'i'),
      ];
    ...
    function buildTradeFromUrl() {
      ...
      // Parse and add picks (format: fromTeamIdx:pickTeam:year:round:destIdx[s],...)
      // Supports both old format (team slugs, "1st"/"2nd") and new compact format (abbreviations, "1"/"2")
      // 's' at end of destIdx indicates a swap
      if (picksParam) {
        ...
        pickStrings.forEach(pickStr => {
          ...
          if (parts.length >= 4) {
            ...
            const year = parts[2];
            let round = parts[3];
            let destPart = parts[4] || '';
            
            // Check for swap flag at end of destination
            const isSwap = destPart.endsWith('s');
            if (isSwap) {
              destPart = destPart.slice(0, -1); // Remove 's' from end
            }
            const destIdx = destPart !== '' ? parseInt(destPart) : undefined;
    ...
    function processCSVData(results) {
      players = results.data
        ...
        .map(row => {
          ...
          // New: Check for expiring contracts and options
          const contractEnd = row['END']?.trim() || '';
          const status2627 = row['ST 26-27']?.trim().toUpperCase() || '';
          const salary2627 = parseSalary(row['2027'] || 0);
          ...
          const hasOption = hasTeamOption || hasPlayerOption;
          
          // Determine trade availability
          let tradeAvailable = row['RESTRICTIONS']?.trim() || '';
          if (is10Day) {
            tradeAvailable = 'not available';
          } else if (isExpiring) {
            tradeAvailable = 'not available'; // Expiring contracts can't be traded after deadline
          }
          
          return {
            player: row.PLAYER?.trim() || '',
            team: row.TEAM?.trim() || '',
            salary: parseSalary(row['2026'] || row['SALARY']),
            contractStatus: contractStatus,
            teamStatus: row['TEAM STATUS']?.trim() || '',
            capSpace: parseSalary(row['CAP SPACE'] || row['Cap Space'] || row['K'] || 0),
            ...
            // New fields for offseason
            salary2627: salary2627,
            status2627: row['ST 26-27']?.trim() || '',
